...
export function errorHandler(err: any, req: Request, res: Response, _next: NextFunction) {
  ...
  if (err.name === "ValidationError") {
    ...
    const error = new ErrorResponseUtil().setError(messages || "Validation failed. Please check your input.")
    res.status(StatusCodes.BAD_REQUEST).json(error)
    return
  }
  ...
  if (err.name === "CastError") {
    const error = new ErrorResponseUtil().setError("Invalid identifier format. Please check your request.")
    res.status(StatusCodes.BAD_REQUEST).json(error)
    return
  }
  ...
  if (err.type === "entity.parse.failed") {
    const error = new ErrorResponseUtil().setError("Invalid request body. Please check your input format.")
    res.status(StatusCodes.BAD_REQUEST).json(error)
    return
  }
